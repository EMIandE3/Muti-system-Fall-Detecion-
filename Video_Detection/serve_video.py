...
def receive_video(port, img_queue):
    ...
    s.listen(True)  # 开始监听TCP传入连接,监听别人的链接，
    ...
    while True:
        ...
        if isinstance(length, bytes):  # 若成功接收到大小信息，进一步再接收整张图片
            ...
            r_img = decimg.reshape(480, 640, 3)  # 给他整形,r_img就是最终传输过来的图像
            img_queue.put(r_img)  # 入队
        ...
        # if cv2.waitKey(10) == 27:  # 如果要显示图像的话，这个不是多余的，但接收不显示的话，就可不用它
        #     break
